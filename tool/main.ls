require! <[fs fs-extra request iconv-lite request]>
#request.debug = true
...
fetch-params = -> new Promise (res, rej) ->
  request {
    ...
    b = iconv-lite.decode(Buffer.from(b), \big5).split \<td
...
fetch-vars = (params) -> new Promise (res, rej) ->
  (e,r,b) <- request {
  ...
  ret = iconv-lite.decode(Buffer.from(b), \big5)
  url = /url=([^']+)/.exec(ret)
...
fetch-data = (matrix, length) -> new Promise (res, rej) ->
  (e,r,b) <- request {
